    ...
    async def on_step(self, iteration: int):
        ...
        if self.strategy_num == 1:
            if ccs.exists:  # 사령부가 하나이상 존재할 경우
                ...
                if self.vespene >= 100 and banshees.amount == 0 and self.time < 17:
                ...
                elif self.time>=36 and self.time < 40 and self.vespene >= 75:
                    if self.time - self.evoked.get((cc.tag, 'train'), 0) > 1.0 :
                        actions.append(cc.train(UnitTypeId.VIKINGFIGHTER))
                        actions.append(cc.train(UnitTypeId.VIKINGFIGHTER))
                        self.evoked[(cc.tag, 'train')] = self.time

                elif self.time > 36 and self.time <= 40 and self.minerals >= 400:
                        actions.append(cc.train(UnitTypeId.HELLION))

                elif self.time > 40:
                    if self.vespene < 75 and self.minerals >= 250 and self.time <60:
                        if self.time - self.evoked.get((cc.tag, 'train'), 0) > 1.0 :
                            actions.append(cc.train(UnitTypeId.HELLION))
                            self.evoked[(cc.tag, 'train')] = self.time

                    elif self.vespene >=75 :
                        actions.append(cc.train(UnitTypeId.VIKINGFIGHTER))
        ...
        for raven in ravens:
            ...
            if self.known_enemy_units.exists:           
                if self.known_enemy_units(UnitTypeId.BANSHEE).exists and self.time < 60 and self.units(UnitTypeId.AUTOTURRET).amount == 0 and self.known_enemy_units(UnitTypeId.BANSHEE).center.position.distance_to(ally_cc) < 25:
                    ...
                    turret_target = self.known_enemy_units(UnitTypeId.BANSHEE).closest_to(raven).position
