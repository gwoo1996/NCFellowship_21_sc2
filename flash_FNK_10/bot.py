    ...
    async def on_step(self, iteration: int):
        ...
        for ghost in ghosts:        
            if enemy_cc[0] > 64:
                ...
                actions.append(ghost(AbilityId.TACNUKESTRIKE_NUKECALLDOWN, target = enemy_cc + (10.5, 0)))
                #actions.append(ghost(AbilityId.TACNUKESTRIKE_NUKECALLDOWN, target= Point2((,31.5))))
            ...
            if enemy_cc[0] > 64 and ghost.distance_to(Point2((73, 31.5 ))) > 0.6:
                actions.append(ghost(AbilityId.MOVE, target = Point2((73, 31.5 ))) )

            elif enemy_cc[0] < 64 and ghost.distance_to(enemy_cc + (22.5, 0)) > 0.3:
                actions.append(ghost(AbilityId.MOVE, target = enemy_cc + (22.5, 0)) )
