...
class Bot(sc2.BotAI):
    ...
    async def on_step(self, iteration: int):
        ...
        #     print('SiegeTank sieged : ')
        #     print( self.enemy_counter[UnitTypeId.SIEGETANKSIEGED])
        #     print('SiegeTank : ')
        #     print( self.enemy_counter[UnitTypeId.SIEGETANK])

        #     print('Battle Cruiser : ')
        #     print(self.enemy_counter[UnitTypeId.BATTLECRUISER])

        #     self.time_flag = self.time

                
        
        enemy_units_amount = self.known_enemy_units.not_structure.amount
        enemy_marines_amount = self.known_enemy_units(UnitTypeId.MARINE).amount
        enemy_battles_amount = self.known_enemy_units(UnitTypeId.BATTLECRUISER).amount
        enemy_sieges_amount = self.known_enemy_units(UnitTypeId.SIEGETANK).amount + self.known_enemy_units(UnitTypeId.SIEGETANKSIEGED).amount
        enemy_vikings_amount = self.known_enemy_units(UnitTypeId.VIKINGFIGHTER).amount + self.known_enemy_units(UnitTypeId.VIKINGASSAULT).amount
        
        if self.enemy_counter[UnitTypeId.BATTLECRUISER] >= 1:
            self.strategy_num = 2
            if self.enemy_counter[UnitTypeId.VIKINGASSAULT] + self.enemy_counter[UnitTypeId.VIKINGFIGHTER] > self.units(UnitTypeId.VIKINGFIGHTER).amount + self.units(UnitTypeId.VIKINGASSAULT).amount:
                self.strategy_num = 4

        elif self.enemy_counter[UnitTypeId.SIEGETANKSIEGED] + self.enemy_counter[UnitTypeId.SIEGETANK] >= 1:
            self.strategy_num = 3
            if self.enemy_counter[UnitTypeId.VIKINGASSAULT] + self.enemy_counter[UnitTypeId.VIKINGFIGHTER] > self.units(UnitTypeId.VIKINGFIGHTER).amount + self.units(UnitTypeId.VIKINGASSAULT).amount:
                self.strategy_num = 4
        
        elif self.enemy_counter[UnitTypeId.VIKINGASSAULT] + self.enemy_counter[UnitTypeId.VIKINGFIGHTER] > self.units(UnitTypeId.VIKINGFIGHTER).amount + self.units(UnitTypeId.VIKINGASSAULT).amount:
            self.strategy_num = 4
        ...
        # 타겟 설정
        if battlecruisers.amount >= 2 and marines.amount >= 30 or self.fast_nuke:
        ...
        if self.strategy_num == 3 or self.enemy_counter[UnitTypeId.SIEGETANKSIEGED] + self.enemy_counter[UnitTypeId.SIEGETANK] >= 1:
            ...
            for marine in marines:
                ...
                if self.time - self.Nuke_calldown_time > 10 and self.Nuke_calldown == True:
                    ...
                    actions.append(marine(AbilityId.ATTACK, target =  target))
        
            

        # 바이킹 명령 생성
        for viking in viking_fighters:
            if self.known_enemy_units.flying.exists:
                flying_enemys = self.known_enemy_units.flying    
                viking_target = flying_enemys.closest_to(viking_fighters[0])

                
                if viking_target.radius + 8.9 + viking.radius > viking.distance_to(viking_target):
                    actions.append(viking.move(ally_cc))
                else :
                    if self.known_enemy_units(UnitTypeId.MARINE).exists:
                        ...
                        if viking.distance_to(enemy_marine) < 6:
                            actions.append(viking.move(ally_cc))
                        else:
                ...
                if self.enemy_counter[UnitTypeId.VIKINGFIGHTER] + self.enemy_counter[UnitTypeId.BATTLECRUISER] + self.enemy_counter[UnitTypeId.BANSHEE] > 0:
        ...
        for raven in ravens:
            enemys = self.known_enemy_units
            for enemy in enemys:
                if enemy.is_cloaked and raven.distance_to(self.units.center) < 10:
                    actions.append(raven.move(enemy.position))
                else:
                    actions.append(raven.move(ally_cc))
            if self.known_enemy_units.exists != True:
                actions.append(raven.move(ally_cc))
