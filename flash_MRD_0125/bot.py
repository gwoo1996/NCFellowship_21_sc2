...
class Bot(sc2.BotAI):
    ...
    async def on_step(self, iteration: int):
        ...
        marauders = self.units(UnitTypeId.MARAUDER)             # 불곰 검색 할당       
        ...
        if self.strategy_num == 1:
            if ccs.exists:  # 사령부가 하나이상 존재할 경우
                ...
                if self.minerals >= 250 and self.time < 21 or (self.minerals >= 200 and self.time > 45):

                    if self.time < burst_time and self.known_enemy_units(UnitTypeId.BANSHEE).exists:
                        if self.known_enemy_units(UnitTypeId.BANSHEE).closest_to(ally_cc).distance_to(ally_cc) < 12:
                            ...
                            if self.can_afford(UnitTypeId.MARAUDER) and self.time > 45:
                                if self.time - self.evoked.get((cc.tag, 'train'), 0) > 1.0:
                                    actions.append(cc.train(UnitTypeId.MARAUDER))
                                    self.evoked[(cc.tag, 'train')] = self.time
                                else:
                                    if self.time - self.evoked.get((cc.tag, 'train'), 0) > 1.0 :
                                        actions.append(cc.train(UnitTypeId.HELLION))
                    ...
                    elif self.can_afford(UnitTypeId.BANSHEE) and self.time < 20 :
                        if self.time - self.evoked.get((cc.tag, 'train'), 0) > 1.0:
                    ...
                    elif self.can_afford(UnitTypeId.MARAUDER) and self.time > 45:
                        if self.time - self.evoked.get((cc.tag, 'train'), 0) > 1.0:
                            actions.append(cc.train(UnitTypeId.MARAUDER))
                            self.evoked[(cc.tag, 'train')] = self.time
                        else:
                ...
                if cc.energy >= 50 and cc.health < 250 and self.units(UnitTypeId.MULE).amount == 0:
                    if self.time - self.time_flag > 3:
            ...
            if self.time < 50:
                ...
                if self.known_enemy_units.not_structure.not_flying.exists:
                    ...
                    if self.time > burst_time :
                        ...
                        actions.append(banshee.attack(ally_cc))
                ...
                if self.known_enemy_units.visible.flying.exists :
                    ...
                    if viking.distance_to(flying_enemys.closest_to(viking)) < 20:
                        ...
                        if self.known_enemy_units(UnitTypeId.MARINE).exists:
                            ...
                            if viking.distance_to(enemy_marine) < 8.0:
        ...
        for marauder in marauders:
            if self.time < burst_time:
                actions.append(marauder.attack(ally_cc))
            else:
                if self.known_enemy_units.not_structure.not_flying.exists:
                    enemy = self.known_enemy_units.not_structure.not_flying.closest_to(marauder)
                    actions.append(marauder.attack(enemy))
                else:
                    actions.append(marauder.attack(target))  

            if self.known_enemy_units.not_structure.not_flying.exists:
                closest_enemy = self.known_enemy_units.not_structure.not_flying.closest_to(marauder)
                if self.known_enemy_units.not_structure.not_flying.amount >= 5 and marauder.distance_to(closest_enemy) < 8.0:
                    if self.time - self.evoked.get((marauder.tag, 'stimpack'), 0) > 15.0:
                        if marauder.health_percentage > 0.5:
                            actions.append(marauder(AbilityId.EFFECT_STIM))
                            self.evoked[(marauder.tag, 'stimpack')] = self.time         
        ...
        for mule in mules:
            if self.units(UnitTypeId.COMMANDCENTER).exists:
                cc = self.units(UnitTypeId.COMMANDCENTER).closest_to(mule)
                if cc.health <= 1500:
                    actions.append(mule.repair(cc))
